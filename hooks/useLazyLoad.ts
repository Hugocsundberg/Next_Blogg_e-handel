...
const useLazyLoad = <T>(query:string | undefined, incrementBy: number, initialResult?:(Array<Product> | Array<PostLight>)) => {
    const [result, setResult] = useState<Array<Product> | Array<PostLight> | []>(initialResult ?? []);
        ...
        if(hasMore && query) {
            ...
            .then((data:Array<Product> | Array<PostLight>) => {
                ...
                setResult((prevData): Array<Product> | Array<PostLight> => {
                    ...
                    if('lastReservedAt' in prevData[0] && 'lastReservedAt' in data[0]) {
