  ...
  const formatDate = (date: Date): string => {
    if (process.browser) {
      ...
      if (inHours <= 1) {
        ...
        return `${Math.round(inHours / 24 / 7)} ${
          weeks === 1 ? "vecka" : "veckor"
        } sedan`;
      } else {
    ...
    >
      ...
      <picture>
        <source
          ...
          srcSet={urlFor(imageRef).width(700).url() ?? imageRef}
        />
        <source
          media={`(min-width:${breakPoints.M}px)`}
          srcSet={urlFor(imageRef).width(560).url() ?? imageRef}
        />
        <source
        ...
        <Image
          aspectRatio={aspectRatio}
          src={urlFor(imageRef).width(740).url() ?? imageRef}
        />
      </picture>
