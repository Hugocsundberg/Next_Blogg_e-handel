...
const Border = styled.div<{
  ...
  box-shadow: ${(props) =>
    props.hasShadow ? "6px 6px 15px -5px #272727c4" : ""};
...
const Dot = styled.div<{
  color: "red" | "yellow" | "transparent";
  sold: boolean;
  reserved: number | false;
}>`
  border-radius: 50%;
  background: ${(props) => {
  ...
  bottom: 1rem;
  box-shadow: ${(props) =>
    props.sold || props.reserved ? "1px 1px 10px" : "none"};
  right: 1rem;
  height: 0.6rem;
  width: 0.6rem;
  z-index: 20;
...
export const Product = ({
  ...
  return (
    <div ref={lastElementRef || null}>
      <Link
        scroll={sold || _isReserved ? false : true}
        href={sold || _isReserved ? "" : `/atelje/${slug}`}
      >
        <Border
          pointer={!sold && !_isReserved}
          hasShadow={hasShadow}
        ...
        >
          ...
          <Overlay active={_isReserved || sold ? true : false}>
            <OverlayText>
              {sold
                ? "Såld"
                : `Reserverad av någon i ${
                    _isReserved ? _isReserved : "false"
                  } ${_isReserved === 1 ? "minut" : "minuter"} till.`}
            </OverlayText>
          </Overlay>
          <picture>
            <source
              ...
              srcSet={
                urlFor(images[0].asset._ref).width(1200).url() || undefined
              }
            />
            <source
              media={`(min-width:${breakPoints.L}px)`}
              srcSet={
                urlFor(images[0].asset._ref).width(500).url() || undefined
              }
            />
            <source
              media={`(min-width:${breakPoints.M}px)`}
              srcSet={
                urlFor(images[0].asset._ref).width(310).url() || undefined
              }
            />
            <source
              media={`(min-width:${breakPoints.S}px)`}
              srcSet={
                urlFor(images[0].asset._ref).width(320).url() || undefined
              }
            />
            <Image
              fadeIn={fadeIn}
              ...
              src={urlFor(images[0].asset._ref).width(500).url() || undefined}
            />
          </picture>
        </Border>
      </Link>
