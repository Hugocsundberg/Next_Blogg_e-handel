...
const ActionButtonCart = ({
  ...
  deliveryDeactivate: () => void;
  deliveryActivate: () => void;
  price: number;
  delivery: boolean;
    ...
    <>
      <Background className="bottomOverlay">
        <FlexColDiv>
          <FlexDiv>
            <p>
              <b>Hämta upp</b> <i>Gratis</i>
            </p>
            <Input
              checked={!delivery}
              type="radio"
              name="delivery"
              onChange={deliveryDeactivate}
            />
          </FlexDiv>
          <FlexDiv>
            <p>
              <b>Skicka med posten</b>{" "}
              <i>{`${deliveryPrice * getFromStorage("cart").length} kr`}</i>
            </p>
            <Input
              checked={delivery}
              type="radio"
              name="delivery"
              onChange={deliveryActivate}
            />
          </FlexDiv>
        </FlexColDiv>
        <P>{`Total ${price} kr`}</P>
        <Button onClick={onClick} color="white" bgcolor="rgb(245,106,141)">
          Checka ut med Klarna
        </Button>
      </Background>
    </>
  );
