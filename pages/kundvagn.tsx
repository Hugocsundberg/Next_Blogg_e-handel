...
const CartContainer = styled.div<{
  ...
  min-height: ${(props) =>
    props.settings.message
      ? "initial"
      : "calc(100vh - ${props=>props.topOverlayHeight}px)"};
  padding: ${margin}rem;
  padding-top: 0;
...
const CheckoutContainerContainer = styled.div<{
  topOverlayHeight: number;
  isVisible: boolean;
}>`
  width: 100vw;
...
const index = ({ settings }: { settings: string }) => {
  ...
  const [KlarnaCheckout, setKlarnaCheckout] =
    useState<KlarnaCheckoutSnippetResponse>();
  ...
  useEffect(() => {
    if (KlarnaCheckout) renderSnippet(KlarnaCheckout.htmlSnippet);
  }, [KlarnaCheckout]);
  ...
  return (
    <>
      ...
      <Background>
        ...
        <CartContainer
          stuffInCart={inCart.length > 0}
          settings={_settings}
          topOverlayHeight={topOverlayHeight}
        >
          {inCart.length > 0 ? (
            inCart.map((product: Object, i) => {
              const _product = product as Product;
              return <CartItem key={i} product={_product} />;
            })
          ) : (
            <NothingInCart />
          )}
          ...
          KlarnaCheckout ? (
            ...
            <ButtonContainer>
              <ActionButtonCart
                ...
                deliveryActivate={deliveryActivate}
                deliveryDeactivate={deliveryDeactivate}
                delivery={delivery}
